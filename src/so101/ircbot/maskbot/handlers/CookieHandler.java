...
public class CookieHandler implements ICommandHandler 
{
	...
	{
		if (!(message.toLowerCase().contains(IRCBot.getInstance().nick.toLowerCase())) && !IRCBot.getInstance().mutedChannels.contains(sender.channelName) && (message.toLowerCase().contains("cookie") || message.toLowerCase().contains("cookies")))
		{
